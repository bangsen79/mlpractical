...
class AffineLayer(LayerWithParameters):
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        """
        #TODO write your code here
        #grads_wrt_weight = np.sum(np.dot(grads_wrt_outputs,inputs)) Ini yang salah
        #grads_wrt_bias = np.sum(grads_wrt_outputs) #ini ayng salah
        #Salahnya itu : Salah Matrix perkalian + Sumnya kecepetan, harusnya sum nya setelah menghitung gradien masing2, bukan pas abis dot product
        grads_wrt_weight = np.dot(grads_wrt_outputs.T,inputs) 
        #ini harus di tranpose pokoknya, ga ke transpose karena ilusi aja mentang2 shapenya simetris
        grads_wrt_bias = np.sum(grads_wrt_outputs, axis=0)
        return grads_wrt_weight , grads_wrt_bias
        raise NotImplementedError()
